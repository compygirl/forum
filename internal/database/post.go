...
func (postObj *PostRepoImpl) CreatePostRepo(post *models.Post) (int64, error) {
	result, err := postObj.db.Exec(`
		INSERT INTO posts (user_id, title, content, created_time, likes_counter, dislikes_counter) VALUES (?, ?, ?, ?, ?, ?);`,
		post.UserID, post.Title, post.Content, post.CreatedTime, post.LikesCounter, post.DislikeCounter)
	if err != nil {
		return -1, err
	}
